 ...
 /**
	...
	*/
	function add_funeral_post()
	{
		...
		$mandatory_key = array('userToken','name','serviceType','dateOfBirth','dateOfDeath','guest' );
		if(empty($_FILES['imageUrl']['name'])) {
			$response['message'] = "Image not found.";
			$response['status'] = "201";
			$this->response($response, 200); // 200 being the HTTP response code
	...
	*/
	function update_funeral_post()
	{
		$x		= stripslashes($this->input->post('data'));
        $data	= json_decode($x,true);
		$mandatory_key = array('userToken','funeralId','name','serviceType','dateOfBirth','dateOfDeath' );
