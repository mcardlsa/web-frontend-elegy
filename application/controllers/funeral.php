        ...
        if($this->user_auth->is_logged_in())
        {
	        ...
	        if($this->form_validation->run())
	        {
	            ...
	            if(!empty($_POST['event1'][0]))
	            {
					...
					//$gmttime=strtotime($val['time']);					
					$user_timezone = $_COOKIE['utimezone'];
					$user=($user_timezone*60*60);
					$orderLocalTime=$t-$user;
	            	$time =date('Y-m-d H:i:s',$orderLocalTime);
					//pr($time);die;
